...
while True:
    if updatesettings:
        ...
        with open('Settings/General.txt', 'r') as setting:
            ...
            if text[5].startswith("true"):
                upadteovernight = True
            else:
                upadteovernight = False
    try:
        conn, addr = s.accept()
        wemode = True
        webmodec = 0
        ...
        webmodec+=1
        functions.updatepx(Digits, animationspeed, rgb, rgbd, brightness, animated)
        Digits = functions.MakeDigits(Date[0], Date[1])
        Date = functions.setupTime()
        if Date[0] == 00 and Date[1] == 00 and upadteovernight:
            machine.reset()
        if Date[1] == 00 and updatetime:
            updatetime = False
            try:
                functions.ntptime.settime()
            except:
                print("Time sync failed, error skiped")
        if Date[1] == 01:
            updatetime = True
        countfornc += 1
